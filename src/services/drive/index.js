...
const copyAndUploadToDrive = async (
  ...
  if (fileStats.isDirectory()) {
    ...
    try {
      ...
      console.log(`Directory ${directoryName} uploaded`);
    } catch (err) {
    ...
    for (const file of files) {
      ...
      await copyAndUploadToDrive(drive, filePath, directoryId, false);
    }
  } else {
...
export const uploadFolder = async (sourceFilePath, Name) => {
  ...
  await copyAndUploadToDrive(drive, sourceFilePath, parentFolderId, true);
